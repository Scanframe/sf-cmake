...
report_url="${NEXUS_SERVER_URL}/repository/${NEXUS_EXCHANGE_REPO}/gitlab-ci/${CI_PROJECT_PATH}/pipeline/${CI_PIPELINE_ID}/report.html"

# URL to the pipeline generating the report.
pipeline_url="${CI_SERVER_URL}/${CI_PROJECT_PATH}/-/pipelines/${CI_PIPELINE_ID}"
...
if cat <<EOF | curl \
	...
	--data-binary @- \
	--no-buffer \
	"${CI_API_V4_URL}/projects/${CI_MERGE_REQUEST_PROJECT_ID}/merge_requests/${CI_MERGE_REQUEST_IID}/notes" >/dev/null; then
{
	"body": "[Coverage report](${report_url} \"Link to coverage report on nexus.\") generated by [pipeline-${CI_PIPELINE_ID}](${pipeline_url} \"Link to responsible pipeline.\") is available."
}
EOF
	WriteLog "Adding coverage comment succeeded."
else
	WriteLog "Adding coverage comment failed [$?]!"
fi
