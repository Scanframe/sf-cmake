...
function DownloadFiles {
	local response_code curl_cmd curl_cmd_add file files fn token url token_url
	# URL to download from.
	url="${NEXUS_SERVER_URL}/service/rest/v1/search?repository=$(UrlEncode "${NEXUS_EXCHANGE_REPO}")&group=/$(UrlEncode "${remote_dir}")"
	# Curl command to execute basically.
	...
	#"${flag_debug}" &&	curl_cmd+=(--verbose)
	curl_cmd+=(--user "${NEXUS_USER}:${NEXUS_PASSWORD}")
	# Set the initial url without a token.
	token_url="${url}"
	# Initialize files array.
	files=()
	# Loop as long as there are continuations to get.
	while true; do
		# Create temporary file to store the json result.
		file="$(mktemp --suffix '.json')"
		# Add the file to teh list
		files+=("${file}")
		# Write output to file.
		curl_cmd_add=(--output "${file}")
		# Perform curl.
		"${curl_cmd[@]}" "${curl_cmd_add[@]}" "${token_url}"
		# Get the possible continuation token.
		token="$(jq -r ".continuationToken" "${file}")"
		# Check if debug reporting is requested.
		if "${flag_debug}"; then
			echo "Token[${file}]: ${token}"
			#echo -e "Response JSON:\n"; cat "${file}"
		fi
		# When no token break the loop.
		if [[ -z "${token}" || "${token}" == 'null' ]]; then
			break
		else
			token_url="${url}&continuationToken=${token}"
		fi
	done
	...
	done < <(jq -r ".items[].assets[]|\"\\(.path)\\t\\(.fileSize)\\t\\(.downloadUrl)\\t\\(.checksum.sha1)\"" "${files[@]}")
}
