...
if (SF_WIN32)
	set(CPACK_PACKAGE_INSTALL_DIRECTORY "${SF_PROVIDER_NAME}/qt/${SF_QT_VERSION}/${_CompilerSubDir}")
	set(CPACK_PACKAGING_INSTALL_PREFIX "/${SF_PROVIDER_NAME}/qt/${SF_QT_VERSION}/${_CompilerSubDir}")
else ()
	set(CPACK_PACKAGE_INSTALL_DIRECTORY "${SF_PROVIDER_NAME}/qt/${SF_QT_VERSION}/${_CompilerSubDir}")
	set(CPACK_PACKAGING_INSTALL_PREFIX "/opt/${SF_PROVIDER_NAME}/qt/${SF_QT_VERSION}/${_CompilerSubDir}")
endif ()
...
if (CPACK_GENERATOR STREQUAL "DEB")
	...
	foreach (_Component IN LISTS CPACK_COMPONENTS_ALL)
		...
		if (_SubComponent STREQUAL "dev")
			...
			set("CPACK_DEBIAN_${_ComponentUpper}_PACKAGE_DEPENDS" "${SF_QT_PACKAGE_FILENAME_PREFIX}rt-${SF_TOOLCHAIN_STRING} (= ${SF_QT_VERSION}-${QT_TWEAK_VERSION})")
		else ()
		...
		set("CPACK_DEBIAN_${_ComponentUpper}_PACKAGE_NAME" "${SF_QT_PACKAGE_FILENAME_PREFIX}${_SubComponent}-${SF_TOOLCHAIN_STRING}")
...
if (CPACK_GENERATOR IN_LIST SF_SUPPORTED_ARCHIVE_GENERATORS)
	...
	foreach (_Component IN LISTS CPACK_COMPONENTS_ALL)
		...
		set("CPACK_ARCHIVE_${_ComponentUpper}_FILE_NAME" "${SF_QT_PACKAGE_FILENAME_PREFIX}${_SubComponent}-${SF_QT_VERSION}_${SF_TOOLCHAIN_STRING}_${SF_QT_VERSION}-${QT_TWEAK_VERSION}_${_Architecture}")
		if (CPACK_GENERATOR STREQUAL "ZIP" AND EXISTS "${SF_ZIP_QT_MANIFEST_FILE}")
			message("${SF_ZIP_QT_MANIFEST_FILE}" "${CPACK_OUTPUT_FILE_PREFIX}/${CPACK_ARCHIVE_${_ComponentUpper}_FILE_NAME}.zip-def")
			file(COPY_FILE "${SF_ZIP_QT_MANIFEST_FILE}" "${CPACK_OUTPUT_FILE_PREFIX}/${CPACK_ARCHIVE_${_ComponentUpper}_FILE_NAME}.zip-def")
		endif ()
	endforeach ()
endif ()
...
if (CPACK_GENERATOR STREQUAL "NSIS64")
	...
	set(CPACK_NSIS_PACKAGE_NAME "${SF_QT_PACKAGE_FILENAME_PREFIX}${SF_TOOLCHAIN_STRING}_${SF_QT_VERSION}-${QT_TWEAK_VERSION}-${SF_ARCHITECTURE}")
	set(CPACK_NSIS_DISPLAY_NAME "${SF_QT_PACKAGE_FILENAME_PREFIX}${SF_TOOLCHAIN_STRING}_${SF_QT_VERSION}-${QT_TWEAK_VERSION}-${SF_ARCHITECTURE}")
	set(CPACK_PACKAGE_FILE_NAME "${SF_QT_PACKAGE_FILENAME_PREFIX}${SF_TOOLCHAIN_STRING}_${SF_QT_VERSION}-${QT_TWEAK_VERSION}_${SF_ARCHITECTURE_SAFE}")
